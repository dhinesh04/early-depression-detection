...
# Split dataset
train_data, val_data = train_test_split(data, test_size=0.3, stratify=[x["label"] for x in data], random_state=42)
# train_loader = DataLoader(MentalHealthDataset(train_data), batch_size=4, shuffle=True)
train_loader = DataLoader(MentalHealthDataset(train_data,tokenizer), batch_size=4, shuffle=True)#tokenizer
...
# Optimizer
optimizer = AdamW(model.parameters(), lr=1e-5)#3e-5#2e-5
...
with torch.no_grad():
    for batch in tqdm(val_loader, desc="Evaluating"):
        batch = {k: v.to(device) for k, v in batch.items()}
        outputs = model(input_ids=batch["input_ids"], attention_mask=batch["attention_mask"],global_attention_mask=batch["global_attention_mask"])
