...
train_loader = DataLoader(MentalHealthDataset(train_data, tokenizer), batch_size=4, shuffle=True)
val_loader = DataLoader(MentalHealthDataset(val_data, tokenizer), batch_size=4)

# Load model
# model = AutoModelForSequenceClassification.from_pretrained("EleutherAI/bert-large-uncased", num_labels=2)
model = AutoModelForSequenceClassification.from_pretrained("chandar-lab/NeoBERT", num_labels=2)
model.to(device)
